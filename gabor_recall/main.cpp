...
std::vector< std::vector< float> > getPixelJet( cv::Mat jet, int rows, int x, int y){
...
//                cv::blur(jet_vector[i * rotations + j], wavelet_blured,cv::Size(13,13));
            ...
            theta_result.push_back( jet_vector[i * rotations + j].at<float>( x, y )  );
        }
...
//              gaborJets.push_back( getPixelJet( gaborResponse, rows, x, y ) );

              cv::circle(transformed_image, kp.pt, cvRound(10), cv::Scalar(255,255,0), -1, 8, 0);
