...
main (int argc, char** argv)
{
    ...
    cv::Mat image_color = cv::imread("../frosties_vpangle/img8.ppm");

    cv::imshow("color image", image_color);


    cv::Mat depth_color = cv::imread("../frosties_vpangle/depth8.pgm", -1);

    cv::imshow("depth image", depth_color);

    cv::waitKey();

    pcl::PointCloud<pcl::PointXYZRGBA> cloud(image_color.cols, image_color.rows);
...
//            int hc = image_color.cols -h - 1;
    ...
    pcl::io::savePCDFile("test_pcl.pcd", cloud);
