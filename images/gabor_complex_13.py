...
def mkKernel(ks, sig, th , lm, ps, gm):
        ...
        xs=np.linspace(-1*101/10.,1*101/10.,101)
        ys=np.linspace(-1*101/10.,1*101/10.,101)
...
#        ys=np.linspace(-1*ks,1*ks,ks)
        ...
        gabor_kernel =  np.array( (1/(sigma**2))*np.exp(-(x_theta**2 + y_theta**2)/(2*sigma**2) )*np.cos(2.*np.pi*x_theta/lmbd + psi), dtype=np.float32)
        gabor_kernel = cv2.resize(gabor_kernel, dsize = (ks,ks) )        
        return gabor_kernel
...
#        ys=np.linspace(-1*ks/10.,1*ks/10.,ks)
        ...
        gabor_kernel =  np.array( (1/(sigma**2))*np.exp(-(x_theta**2 + y_theta**2)/(2*sigma**2) )*np.sin(2.*np.pi*x_theta/lmbd + psi), dtype=np.float32)
        gabor_kernel = cv2.resize(gabor_kernel, dsize = (ks,ks) )        
        return gabor_kernel
