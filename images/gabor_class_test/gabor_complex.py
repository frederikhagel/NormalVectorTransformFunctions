...
def mkKernel(ks, cks,sig, th , lm, ps, gm):
...
def mkKernelI(ks, cks, sig, th , lm, ps, gm):
        if not ks%2:
            exit(1)
...
class complexKernel:
    ...
    def __init__ (self, ks, cks, sig, th, lm, ps, gm):
        self.kernel_real = mkKernel(ks, cks,sig, th, lm, ps, gm )
        self.kernel_imaginary = mkKernelI(ks, cks, sig, th, lm, ps, gm )
