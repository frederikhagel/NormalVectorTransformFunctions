...
//            int hc = image_color.cols -h - 1;
            ...
            cloud->at(h,w).x = float( (h - image_color.cols/2)/1000.0 );
            cloud->at(h,w).y = float( (-w + image_color.rows/2)/1000.0 );
...
//    pcl::flipNormalTowardsViewpoint ( cloud->at( y * image_color.cols + x ), cloud->sensor_origin_[0], cloud->sensor_origin_[1], cloud->sensor_origin_[2], normal.normal_x, normal.normal_y, normal.normal_z);
  ...
  //  std::cout << "cloud_normal at point: " << cloud_normals->at( 0 ) << std::endl;
    ...
    for( int i = 0; i < cloud_normals->size(); i++ )
    {
        ...
        pcl::flipNormalTowardsViewpoint ( cloud->at( y * image_color.cols + x ), cloud->sensor_origin_[0], cloud->sensor_origin_[1], cloud->sensor_origin_[2], z_axis[0], z_axis[1], z_axis[2]);

        std::cout << "Normal vector: " << z_axis << " and " << z_axis.normalized() << std::endl;
...
//        Eigen::Vector3f tmp1 = (z_axis.normalized().cross(x_proj.normalized())).normalized();
        ...
        for(int i = 0; i < 4; i++)
        {
            float xProj = (-1050)* P_0[i].x/(P_0[i].z + distance) + input_cloud.cols/2;
            float yProj =  (1050) * P_0[i].y/(P_0[i].z + distance) + input_cloud.rows/2;
...
//        std::cout << "[ " << P_T[0] << " , " << P_T[1] << " , " << P_T[2] << " , " << P_T[3] << " ]" <<    std::endl;
        ...
        for(int i = 0; i < 4; i++)
        {
            float xProj = (-1050)* P_T[i].x/(P_T[i].z + distance) + x; // input_cloud.cols/2;
            float yProj =  (1050) * P_T[i].y/(P_T[i].z + distance) + y; // input_cloud.rows/2;
